package com.newframe.core.interceptors;
...
//					// request.getRequestDispatcher("pages/common/noAuth.jsp").forward(request,
//					// response);
//					return false;
//				}
...
//							.getOperationCodesByUserIdAndFunctionId(client.getUser().getId(), functionId);
//					request.setAttribute("operationCodes", operationCodes);
//
//				}
//				if (!oConvertUtils.isEmpty(functionId)) {
//					List<String> allOperation = this.systemService.findListbySql(
//							"SELECT operationcode FROM t_s_operation  WHERE functionid='" + functionId + "'");
//
...
//								+ "' AND fun.operation!=''  AND fun.roleid=role.roleid AND role.userid='"
...
//		}
	...
	private boolean hasMenuAuth(HttpServletRequest request) {
		...
		if (functionList == null) {
			functionList = systemService.loadAll(Function.class);
		}
		...
		String userid = ClientManager.getInstance().getClient(ContextHolderUtils.getSession().getId()).getUser()
				.getId();
