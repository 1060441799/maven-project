...
public class HiberAspect extends EmptyInterceptor {
	...
	public boolean onSave(Object entity, Serializable id, Object[] state, String[] propertyNames, Type[] types) {
		...
		Depart depart = commonService.findByProperty(Depart.class, "id", currentUser.getDepartId()).get(0);
		try {
			// 添加数据
			for (int index = 0; index < propertyNames.length; index++) {
				...
				/* 找到名为"创建人部门"的属性 */
					...
					if (oConvertUtils.isEmpty(state[index])) {
						state[index] = currentUser.getDepartId();
					}
					continue;
				}
				/* 找到名为"创建人部门"的属性 */
					...
					/* 使用拦截器将对象的"创建人部门"属性赋上值 */
					if (oConvertUtils.isEmpty(state[index])) {
						state[index] = depart.getDepartname();
					}
					continue;
				}
	...
	public boolean onFlushDirty(Object entity, Serializable id, Object[] currentState, Object[] previousState,
		...
		for (int index = 0; index < propertyNames.length; index++) {
			...
			/* 找到名为"修改人部门"的属性 */
				...
				currentState[index] = depart.getId();
				continue;
			}
			/* 找到名为"修改人部门"的属性 */
			else if ("updateDepartmentname".equals(propertyNames[index])) {
				/* 使用拦截器将对象的"修改人部门"属性赋上值 */
				currentState[index] = depart.getDepartname();
				continue;
			}
		}
		return true;
