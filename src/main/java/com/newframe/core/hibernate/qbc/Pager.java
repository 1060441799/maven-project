...
public class Pager {
	...
	public String toString() {
		return "Pager的值为 " + " curPageNO = " + curPageNO + " limit = " + pageSize + " rowsCount = " + rowsCount
				+ " pageCount = " + pageCount;
	}
	...
	public String getToolBar(String url) {
		...
		if (isFirst())
			...
			str += "第" + curPageNO + "页&nbsp;共" + pageCount + "页&nbsp;<a href='" + url + temp + "curPageNO=1"
					+ conditions + "'>首页</a>&nbsp;";
			str += "<a href='" + url + temp + "curPageNO=" + previous() + conditions
					+ "' onMouseMove=\"style.cursor='hand'\" alt=\"上一页\">上一页</a>&nbsp;";
		}
		if (isLast() || rowsCount == 0)
			str += "下一页 尾页&nbsp;";
		else {
			str += "<a href='" + url + temp + "curPageNO=" + next() + conditions
	...
	public String getToolBar(String myaction, String myform) {
		...
		str += " }" + "\n";
		str += "</script>" + "\n";
		str += "&nbsp;<DIV class=pageArea id=pageArea>共<b>" + rowsCount + "</b>条&nbsp;当前第" + curPageNO + "/" + pageCount
				+ "页&nbsp;&nbsp;&nbsp;";
		if (curPageNO == 1 || curPageNO == 0)
			...
			str += "<a class=pagePrevious title=上一页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit("
					+ (curPageNO - 1) + ")\"></a>";
		}
		if (curPageNO - pageCount == 0 || pageCount == 0 || pageCount == 1)
			...
			str += "<a class=pageNext title=下一页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit("
					+ (curPageNO + 1) + ")\"></a>";
			str += "<a class=pageLast title=尾页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(" + pageCount
					+ ")\"></a>";
		}
