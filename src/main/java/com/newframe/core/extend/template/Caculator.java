package com.newframe.core.extend.template;
...
public class Caculator {
	...
	public String calc() {
		...
		while (!calcStack.isEmpty()) {
			...
			if (!it.ops) {
				...
				case '/':
					...
					newItem.value = op1 / op2;
					if (newItem.value == Double.NEGATIVE_INFINITY || newItem.value == Double.POSITIVE_INFINITY
							|| new Double(newItem.value).toString().equals("NaN")) {
						newItem.value = 0.0;
					}
					// double tt=0.0/0.0;
		...
		/*
		 * double c=0; long b = (long)Math.round(result * 100); //小数点后两位前移，并四舍五入
		 * if((b % 100)!=0) { c = (double)b / 100.00; //还原小数点后两位 return
		 * dfd.format(c); }else { c = (double)b / 100.00; return dfi.format(c);
		 * }
		 */
	}
	 ...
	 * a)循环，当栈非空，并且栈顶元素不是开括号，并且栈顶运算符优先级不低于输入的运算符的优先级，反复将其输出 b）把输入运算符压栈
	 * 5）输出栈内剩余元素
	 * 
	 * @param in
	 * @return
