...
public class ResourceUtil {
	...
	public static final String getSessionUserId() {
		HttpSession session = ContextHolderUtils.getSession();
		if (ClientManager.getInstance().getClient(session.getId()) != null) {
			return ClientManager.getInstance().getClient(session.getId()).getUser().getId();
		}
		return null;
	}
	...
	@Deprecated
	public static final List<RelRoleFunction> getSessionTSRoleFunction(String roleId) {
		HttpSession session = ContextHolderUtils.getSession();
		if (session.getAttributeNames().hasMoreElements()) {
			List<RelRoleFunction> TSRoleFunctionList = (List<RelRoleFunction>) session.getAttribute(roleId);
